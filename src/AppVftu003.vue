...
<template>
  ...
  <div class="pt-page pt-page-current pt-page-controller search-pager">
    <PageHeader ref="pageHeader" :labels="labels" pid="vftu003" version="1.0.0" showLanguage="true" @language-changed="changeLanguage" :multiLanguages="multiLanguages" :build="buildVersion" :visible="displayPageHeader" />
...
export default {
    ...
    this.$nextTick(async () => {
      ...
      startApplication("vftu003",(data) => {
        ...
        if(data.type=="language") {
          ...
          loadAndMergeLabel("vftu003", (success) => {
            if (success) {
  ...
  methods: {
    ...
    dataSelected(item,action) {
      ...
      if("edit"==action) {
        this.$refs.entryForm.retrieveRecord({apiname: item.apiname});
      } else if("delete"==action) {
        this.$refs.entryForm.startDeleteRecord({apiname: item.apiname});
      }
    },
    dataInsert(filters) {
      //listen action from search form
